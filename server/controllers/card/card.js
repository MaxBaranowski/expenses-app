    ...
    _card
      ...
      .then(card => {
        if ( card ) {
          ...
          _card
            .addDailyCardExpense({ user_id : String(req.user._id), date, cards, year, month })
            .then(res => resolve(res))
            .catch(err => reject(err));
        } else {
          // if no card
          _card
            .createDayCard({ user_id : String(req.user._id), date, cards, year, month })
            .then(res => resolve(res))
            .catch(err => reject(err));
        }
      })
    ...
    .then(() => {
      return new Promise((resolve, reject) => {
        _card
          ...
          .then(totalAmmount =>
            _card
              .updateDailyCardTotalAmmount({ user_id : String(req.user._id), date, totalAmmount })
...
module.exports.getYearlyExpenses = function (req, res, next) {
  ...
  return new Promise((resolve, reject) => {
    _card.getFullYearCard({ user_id : String(req.user._id), year })
